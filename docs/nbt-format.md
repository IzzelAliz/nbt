...
NBT (Named Binary Tag) format, which Mojang uses for data reading and writing in Minecraft, does not have a formal specification.

However, the community concluded several conventions today. Following is a brief introduction of NBT formats.

## NBT types

There are 13 different nbt types recognized by Mojang's system. Each nbt type has its own number id:

| Name             | Number Id | Description                                                                                                               |
| ---------------- | --------- | ------------------------------------------------------------------------------------------------------------------------- |
| `TAG_End`        | 0         | Singletion. Not really appear in actual uses. Usually used as fallback when corresponding value does not exist.           |
...
String format of NBT (also called SNBT) usually appears in game commands. For example:

```mcfunction
# summon an enchanted book of mending
...
Elements of a list are wrapped by `[]`, and named tags of a compound are wrapped by `{}`. A named tag use `:` to separate the string name and the tag:
...
Binary format is the actual format which Minecraft uses for loading and saving game data.

The basic component of binary data is named tag, which consists of three parts:
...
Here are some example names tags of number arrays:
...
A compound tag is composed of multiple named tags sequencially and ends with a zero byte (number id of end tag):
...
All the number mentioned above are stored as big endian numbers (store `1s` as `00 01` but not `01 00`).

## Compressed binary format

In most cases, Minecraft stores game data to GZIP compressed binary file instead of uncompressed raw data shown above. However, there are still some data saved without compression (such as `servers.dat`).
